...
def parse(input_filename, output_filename):
    ...
    if input_filename == "-":
        ...
        num_lines = int(subprocess.check_output(
            ["wc", "-l", input_filename]).strip().split()[0])
    tables = {}
    current_table = None
    creation_lines = []
    enum_types = []
    ...
    for i, line in enumerate(input_fh):
        ...
        logging.flush()
        line = line.strip().replace(r"\\", "WUBWUBREALSLASHWUB").replace(r"\0", "").replace(r"\'", "''").replace("WUBWUBREALSLASHWUB", r"\\").replace("`", '"')
        ...
        if current_table is None:
            ...
            # Inserting data into a table?
                ...
                output.write(
                    re.sub(r"([^'])'0000-00-00 00:00:00'", r"\1NULL", line) + "\n")
        ...
        # Inside-create-statement handling
            ...
            if line.startswith('"'):
                ...
                if final_type:
                    cast_lines.append(
                        "ALTER TABLE \"%s\" ALTER COLUMN \"%s\" DROP DEFAULT" % (current_table, name))
                    cast_lines.append("ALTER TABLE \"%s\" ALTER COLUMN \"%s\" TYPE %s USING CAST(\"%s\" as %s)" % (
                        current_table, name, final_type, name, final_type))
                    if final_default:
                        cast_lines.append("ALTER TABLE \"%s\" ALTER COLUMN \"%s\" SET DEFAULT %s" % (
                            current_table, name, final_default))
                # ID fields need sequences [if they are integers?]
                if extra.endswith("AUTO_INCREMENT"): # if name == "id" and set_sequence is True:
            ...
            # Is it a constraint or something?
                ...
                index_lines.append("CREATE UNIQUE INDEX \"%s\" ON %s (%s)" % (
                    index_name, current_table, index_columns))
                drop_index_lines.append(
                ...
                index_lines.append("CREATE INDEX ON %s USING gin(to_tsvector('english', %s))" % (
